...
function DeleteProjectModal({ setModalOpened, opened, project }: Props) {
  ...
  return (
    <Modal opened={opened} onClose={() => closeModal()} title="Delete Project">
      <Title mb={"md"} order={3}>
        Delete this project?
      </Title>
      <Code>{project ? project.projectPath : "..."}</Code>
      <Text size={"sm"} my={"lg"}>
        This will delete the project, its tests, and all associated files from
        your projects directory and the application database. This action cannot
        be undone.{" "}
      </Text>
      <br />
      <TextInput
      ...
      <Group position={"right"} mt={"lg"}>
        ...
        <Button color={"red"} onClick={() => deleteProject()} disabled={!equiv}>
          Delete My Project
        </Button>
      </Group>
    </Modal>
