...
electron_2.ipcMain.on(RUN_TEST, (event, projectKey, testKey, returnChannel) => {
    ...
    let connectionTypes = [
        ...new Set(Object.entries(canvas["connections"]).map((value) => value[1]["connectionLabel"])),
    ];
    ...
    let activePreds = Object.entries(store.get(`projects.${projectKey}.predicates`)).filter(([key, value]) => value.status === "equals" || value.status === "negate");
