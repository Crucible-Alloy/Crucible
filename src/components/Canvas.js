...
export const Canvas = ({ snapToGrid, tab, projectKey, testKey }) => {
  ...
  const addNewAtom = (
    ...
    window.electronAPI
      .getAtomMultiplicity(projectKey, sourceAtomKey)
      .then((multiplicity) => {
        ...
        if (eligibleToAdd) {
          let atomCount = Object.entries(canvasItems["atoms"]).filter(
            ([key, value]) => value["sourceAtomKey"] === sourceAtomKey
          ).length;
  ...
  const updateAtom = (id, left, top, sourceAtomKey, atomLabel, nickname) => {
    window.electronAPI.createAtom(projectKey, testKey, id, {
      top: top,
      left: left,
      sourceAtomKey: sourceAtomKey,
      atomLabel: atomLabel,
      ...
      >
        ...
        {Object.entries(canvasItems["atoms"]).map(([key, value]) => (
          <Atom
            ...
            testKey={testKey}
            sourceAtomKey={value["sourceAtomKey"]}
            label={value["atomLabel"]}
            atomColor={value["color"]}
            {...canvasItems["atoms"][key]}
          />
        ))}
